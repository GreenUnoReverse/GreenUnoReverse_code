...
class kickoff():
    ...
    def run(self, agent):
        ...
        defaultThrottle(agent, 2300)
        # width = 200
        # for thingy in range(len(agent.boosts)): # thingy is boost
        #     if agent.boosts[thingy].location.x < agent.me.location.x + width and agent.boosts[thingy].location.x > agent.me.location.x - width and agent.boosts[thingy].location.y > agent.me.location.y and agent.boosts[thingy].location.y < agent.ball.location.y: # if boost is between car and ball
        #         agent.set_intent(goto(thingy))
        #         return
